...
for dataset_name, epochs in dataset_configs.items():
    ...
    output_dir = f"sample{sample_num}_epochs{epochs}_seed{SEED}"
    ...
    sample_num = min(sample_num, len(train_data))
    train_sample_data = train_data.head(sample_num)
    # Setup GReaT
    ...
    # Train
    if isinstance(train_sample_data, pd.DataFrame):
        trainer = great.fit(train_sample_data)
    else:
        column_names = column_dict[dataset_name]
        trainer = great.fit(train_sample_data, column_names=column_names)
